...
function handleTool(name, args) {
  // KB tools
  if(name==="search_kb"){return getDrives().then(function(drives){var kbLibs=["FAQs","Troubleshooting","Runbooks","Assets","Cabling","Scripts"];var kbDrives=drives.filter(function(d){return kbLibs.indexOf(d.name)!==-1;});if(!kbDrives.length)return{content:[{type:"text",text:"No KB libraries available."}]};return Promise.all(kbDrives.map(function(d){return graph("/drives/"+d.id+"/root/search(q='"+encodeURIComponent(args.query)+"')").then(function(r){return (r.value||[]).map(function(f){return{name:f.name,id:f.id,driveId:d.id,library:d.name};});}).catch(function(){return [];});})).then(function(results){var all=[].concat.apply([],results).slice(0,8);if(all.length)return{content:[{type:"text",text:JSON.stringify(all,null,2)}]};return{content:[{type:"text",text:"No results found in the Knowledge Base for: "+args.query}]};});});}    if(name==="sync_learn_to_kb"){return syncLearnTopicToKB(args.topic,args.library,args.max_articles||3).then(function(r){var msg=r.uploaded>0?"Synced "+r.uploaded+" article(s) to "+r.library+":\n"+r.articles.map(function(a){return"• "+a.title+" ("+a.file+")";}).join("\n"):"Nothing uploaded. Skipped: "+r.skipped+" (no content or upload error).";return{content:[{type:"text",text:msg}]};}).catch(function(e){return{content:[{type:"text",text:"sync_learn_to_kb error: "+e.message}]};});}
