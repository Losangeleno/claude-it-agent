...
function handleTool(name, args) {
  // KB tools
  if(name==="search_kb"){var KB_DRIVES={FAQs:"b!KAb296Hrk0ep9AvCIWq7npLU2tvhB3lCoKSMLhg07d-0YkaK7sToQb9UfBCD0V8l",Troubleshooting:"b!KAb296Hrk0ep9AvCIWq7npLU2tvhB3lCoKSMLhg07d-s9M-vo64gR6RqcavYF4co",Runbooks:"b!KAb296Hrk0ep9AvCIWq7npLU2tvhB3lCoKSMLhg07d8ntgJz28NVQ5IBUqynE4Gk",Assets:"b!KAb296Hrk0ep9AvCIWq7npLU2tvhB3lCoKSMLhg07d9OV5yeNjEWSZzs4VJ2fbAB",Cabling:"b!KAb296Hrk0ep9AvCIWq7npLU2tvhB3lCoKSMLhg07d9lg9HgzNLwT7cu7swCUvqF"};var entries=Object.keys(KB_DRIVES).map(function(lib){return{lib:lib,id:KB_DRIVES[lib]};});return Promise.all(entries.map(function(e){return graph("/drives/"+e.id+"/root/search(q='"+encodeURIComponent(args.query)+"')").then(function(r){return (r.value||[]).map(function(f){return{name:f.name,id:f.id,driveId:e.id,library:e.lib};});}).catch(function(err){console.error("search_kb drive "+e.lib+" failed:",err.message);return [];});})).then(function(results){var all=[].concat.apply([],results).slice(0,8);if(all.length)return{content:[{type:"text",text:JSON.stringify(all,null,2)}]};return{content:[{type:"text",text:"No results found in the Knowledge Base for: "+args.query+" (searched "+entries.length+" drives)"}]};});}  if(name==="sync_learn_to_kb"){return syncLearnTopicToKB(args.topic,args.library,args.max_articles||3).then(function(r){var msg=r.uploaded>0?"Synced "+r.uploaded+" article(s) to "+r.library+":\n"+r.articles.map(function(a){return"• "+a.title+" ("+a.file+")";}).join("\n"):"Nothing uploaded. Skipped: "+r.skipped+" (no content or upload error).";return{content:[{type:"text",text:msg}]};}).catch(function(e){return{content:[{type:"text",text:"sync_learn_to_kb error: "+e.message}]};});}
