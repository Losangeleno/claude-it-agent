...
function handleTool(name, args) {
  // KB tools
  if(name==="search_kb"){return (function(q){return new Promise(function(ok){var ht=require("https"),b=JSON.stringify({query:q}),req=ht.request({hostname:"hook.us2.make.com",path:"/ei14bxc9xiqr6ib1ls45313jji44v32b",method:"POST",headers:{"Content-Type":"application/json","Content-Length":Buffer.byteLength(b)}},function(r){var d="";r.on("data",function(c){d+=c;});r.on("end",function(){try{var j=JSON.parse(d),hits=[];if(j.results&&j.results[0]&&j.results[0].hitsContainers)j.results[0].hitsContainers.forEach(function(hc){(hc.hits||[]).forEach(function(hit){if(hit.resource)hits.push({name:(hit.resource.name||""),library:(hit.resource.webUrl||"").split("/").slice(-2,-1)[0]||"KB",webUrl:(hit.resource.webUrl||"")});});});ok({content:[{type:"text",text:hits.length?JSON.stringify(hits,null,2):"No results found."}]});}catch(e){ok({content:[{type:"text",text:"No results found."}]});} });});req.on("error",function(){ok({content:[{type:"text",text:"No results found."}]});});req.write(b);req.end();})})(args.query||"");}
